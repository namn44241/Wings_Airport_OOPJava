document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("nav-booking")
    .addEventListener("click", () => showContainer("booking"));
  document
    .getElementById("nav-manage")
    .addEventListener("click", () => showContainer("manage"));
  document
    .getElementById("nav-checkin")
    .addEventListener("click", () => showContainer("checkin"));
  ...
  function initializeManage() {
    ...
    const lastRegisteredCustomerId = localStorage.getItem(
      "lastRegisteredCustomerId"
    );
      ...
      .addEventListener("submit", function (e) {
        ...
        fetch(`/api/search?query=${encodeURIComponent(query)}`)
          ...
          .catch((error) => {
            console.error("Error:", error);
            tableBody.innerHTML =
              '<tr><td colspan="7">Lỗi khi tìm kiếm dữ liệu</td></tr>';
          });
      });
  }
    ...
    rows.forEach((row) => {
      ...
      if (type === "flight") {
        ...
        row.style.display =
          ...
          arrivalAirport.toLowerCase().includes(query.toLowerCase())
            ? ""
            : "none";
      }
    });
